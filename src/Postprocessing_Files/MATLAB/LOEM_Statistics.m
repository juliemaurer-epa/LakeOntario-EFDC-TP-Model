...
atmos18 = '/work/GLFBREEZ/LOEM/LOEM_Winmodel_nc/Scenario3/';
output_file1 = 'median_TP_zones_LOEMS3.txt';
output_file2 = 'median_TP_zones_5m_30m_LOEMS3.txt';

FToRead18 = strcat(atmos18,'LOEM_2018_scenario3.nc'); %Read NETCDF
FToRead = FToRead18;

GoMDOM = ncinfo(FToRead);

%%================Read GoMDOM Ontario Output==================
LatG = double(ncread(FToRead,'lat'));
LonG = double(ncread(FToRead,'lon'));
time = ncread(FToRead,'datetime');
celldepth = ncread(FToRead,'depth');
landmask = ncread(FToRead,'dry');
...
tcount=[255 132 10 inf]; %Longitude,Latitude,Depth,Time
tstride=[1 1 1 1]; %Longitude,Latitude,Depth,Time

% Establish limits in space and time for the NETCDF being read for COMT
tstarts=[1 1 1]; %Longitude,Latitude,Depth,Time
tcounts=[255 132 10]; %Longitude,Latitude,Depth,Time
tstrides=[1 1 1]; %Longitude,Latitude,Depth,Time


%Read GoMDOM
TP = double(ncread(FToRead,'TP',tstart,tcount,tstride));

% Set land locations to NaN
for i = 1:255
    for j = 1:132
	if landmask(i,j) == 1
	   TP(i,j,:,:) = NaN;
...
% Set -9999 cells to NaN.
TP(TP < 0) = NaN;

% Set land locations to NaN.
celldepth(landmask == 1) = NaN;
...
for i=1:length(z1latref)
    for p=1:size(TP,4)
    for t=1:10
        dataZ1(i,t,p) = TP(z1lonref(i),z1latref(i),t,p);
    end
    end
end

for i = 1:length(z1latref)
    hZ1(i) = celldepth(z1lonref(i),z1latref(i));
end
...
for i=1:length(z2latref)
    for p=1:size(TP,4)
    for t=1:10
        dataZ2(i,t,p) = TP(z2lonref(i),z2latref(i),t,p);
    end
    end
end

for i = 1:length(z2latref)
    hZ2(i) = celldepth(z2lonref(i),z2latref(i));
end
...
for i=1:length(z3latref)
    for p=1:size(TP,4)
    for t=1:10
        dataZ3(i,t,p) = TP(z3lonref(i),z3latref(i),t,p);
    end
    end
end

for i = 1:length(z3latref)
    hZ3(i) = celldepth(z3lonref(i),z3latref(i));
end
...
for i=1:length(z4latref)
    for p=1:size(TP,4)
    for t=1:10
        dataZ4(i,t,p) = TP(z4lonref(i),z4latref(i),t,p);
    end
    end
end

for i = 1:length(z4latref)
    hZ4(i) = celldepth(z4lonref(i),z4latref(i));
end
	
% Calculate median of TP concentrations at each zone for the summer months (June -- August). 
median_zone1 = median(dataZ1(:,:,62:154), "all", "omitnan");
median_zone2 = median(dataZ2(:,:,62:154), "all", "omitnan");
median_zone3 = median(dataZ3(:,:,62:154), "all", "omitnan");
median_zone4 = median(dataZ4(:,:,62:154), "all", "omitnan");
...
% Calculate median of TP concentrations at each zone for the summer months (June -- August). 
median_zone1a = median(dataZ1a(:,:,62:154), "all", "omitnan");
median_zone1b = median(dataZ1b(:,:,62:154), "all", "omitnan");
median_zone1c = median(dataZ1c(:,:,62:154), "all", "omitnan");

median_zone2a = median(dataZ2a(:,:,62:154), "all", "omitnan");
median_zone2b = median(dataZ2b(:,:,62:154), "all", "omitnan");
median_zone2c = median(dataZ2c(:,:,62:154), "all", "omitnan");

median_zone3a = median(dataZ3a(:,:,62:154), "all", "omitnan");
median_zone3b = median(dataZ3b(:,:,62:154), "all", "omitnan");
if max(hZ3) <= 30
   median_zone3c = NaN
else
   median_zone3c = median(dataZ3c(:,:,62:154), "all", "omitnan");
end
...
median_zone4c = median(dataZ4c(:,:,62:154), "all", "omitnan");
